...
describe("registerAttendee", () => {
  it("creates a role-less account with a hashed password and no email verification", async () => {
    const res = await registerAttendee("New@X.com", "longenough1", "Jane", "en", "test-flow", { kind: "public", origin: "test-origin" });
  ...
  it("rejects a weak password before any DB call", async () => {
    const res = await registerAttendee("a@b.com", "short", undefined, "en", "test-flow", { kind: "public", origin: "test-origin" });
  ...
  it("rejects an invalid email", async () => {
    const res = await registerAttendee("not-an-email", "longenough1", undefined, "en", "test-flow", { kind: "public", origin: "test-origin" });
    expect(res.ok).toBe(false);
    expect(prisma.user.create).not.toHaveBeenCalled();
  });
  ...
  it("answers a taken address exactly as it answers a free one", async () => {
    const free = await registerAttendee("free@x.com", "longenough1", undefined, "en", "test-flow", { kind: "public", origin: "test-origin" });

    vi.clearAllMocks();
    mock(prisma.user.findUnique).mockResolvedValue({ id: "existing", status: "active" });
    const taken = await registerAttendee("dupe@x.com", "longenough1", undefined, "en", "test-flow", { kind: "public", origin: "test-origin" });
  ...
  it("never creates a second account for a taken address", async () => {
    mock(prisma.user.findUnique).mockResolvedValue({ id: "existing", status: "active" });
    await registerAttendee("dupe@x.com", "longenough1", undefined, "en", "test-flow", { kind: "public", origin: "test-origin" });
    expect(prisma.user.create).not.toHaveBeenCalled();
  });

  it("does not leak the result through the userId either", async () => {
    const res = await registerAttendee("new@x.com", "longenough1", undefined, "en", "test-flow", { kind: "public", origin: "test-origin" });
  ...
  it("sends exactly one mail on either branch, so the screen is true both ways", async () => {
    await registerAttendee("free@x.com", "longenough1", undefined, "en", "test-flow", { kind: "public", origin: "test-origin" });
  ...
  it("stays silent for a suspended account but still answers success", async () => {
    ...
    const res = await registerAttendee("suspended@x.com", "longenough1", undefined, "en", "test-flow", { kind: "public", origin: "test-origin" });
    expect(res).toEqual({ ok: true });
...
describe("registerAttendee — work done, not just words said", () => {
  ...
  async function argon2CallsFor(kind: "free" | "taken" | "suspended") {
    ...
    await registerAttendee(`${kind}@x.com`, "longenough1", undefined, "en", "test-flow", { kind: "public", origin: "test-origin" });
  ...
  it("caps signup mail per address so it cannot be used to mailbomb", async () => {
    // Spread across origins, so the ceiling is what stops it rather than the
    // per-origin cap — that one has its own test below.
    for (let i = 0; i < MAIL_LIMIT; i += 1) {
      vi.clearAllMocks();
      mock(prisma.user.findUnique).mockResolvedValue({ id: "existing", status: "active" });
      await registerAttendee("victim@x.com", "longenough1", undefined, "en", "test-flow", { kind: "public", origin: `10.0.0.${i}` });
      expect(sendEmail).toHaveBeenCalledTimes(1);
    }

    vi.clearAllMocks();
    mock(prisma.user.findUnique).mockResolvedValue({ id: "existing", status: "active" });
    await registerAttendee("victim@x.com", "longenough1", undefined, "en", "test-flow", { kind: "public", origin: "10.0.0.99" });
    expect(sendEmail).not.toHaveBeenCalled();

    // A different address is unaffected — the cap is per victim, not global.
    vi.clearAllMocks();
    mock(prisma.user.findUnique).mockResolvedValue({ id: "existing", status: "active" });
    await registerAttendee("someone-else@x.com", "longenough1", undefined, "en", "test-flow", { kind: "public", origin: "10.0.0.99" });
    expect(sendEmail).toHaveBeenCalledTimes(1);
  });
  ...
  it("one origin cannot spend a victim's whole budget through the signup form", async () => {
    const send = async (origin: string) => {
      vi.clearAllMocks();
      mock(prisma.user.findUnique).mockResolvedValue({ id: "existing", status: "active" });
      await registerAttendee("target@x.com", "longenough1", undefined, "en", "test-flow", { kind: "public", origin });
