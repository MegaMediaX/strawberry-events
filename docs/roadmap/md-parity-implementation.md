...
| 2 | Staff walk-in registration | — | `/[locale]/(staff)/staff/events`, `/staff/registrations` | ⏳ |
| 3 | Admin dashboard KPIs + registrations table upgrade | — | (upgrades `/admin`, `/admin/registrations`) | ⏳ |
| 4 | User management | `User.status` enum | `/admin/users`, `/admin/users/[id]` | ⏳ |
| 5 | Email logs + resend | `EmailLog` model | `/admin/emails`, `/admin/emails/[id]` | ⏳ |
| 6 | Modular form fields per ticket | `CustomFormField.placeholderEn/Ar` | (admin event/ticket UI + wizard) | ⏳ |
| 7 | Maps / location | `EventMapping` location cols | (event create/edit + public detail) | ⏳ |
| 8 | Calendar polish | — | `/[locale]/(public)/events/[slug]/calendar.ics` (route handler) | ⏳ |
| 9 | Attendee portal | maybe reset-token | `/register`, `/forgot-password`, `/reset-password`, `/my-registrations`, `/profile` | ⏳ |
| 10 | Arabic UI pass (i18n externalization + RTL) | — | — | ⏳ |
| 11 | Final validation + PR description | — | — | ⏳ |

> Ordering note vs. the brief: the Arabic pass is done **last** so it externalizes strings introduced by chunks 2–9 in one sweep, rather than retrofitting each. New UI in chunks 2–9 is built i18n-key-first to minimize that retrofit.
