...
| **C3** | Secrets hygiene. No real secret is committed (verified: `.env` is gitignored; repo scan found only placeholders/detection-code). `.env.production.example` added (placeholders only) with **rotation + ENCRYPTION_KEY-loss** guidance; `.env.example` gains MAGIC_LINK_SECRET + ALLOW_EMAIL_DISABLED_IN_PRODUCTION. `compose.yaml` secret defaults changed from `:-change_me`/`:-password` to `${VAR:?…}` (fail if unset). | `.env.production.example`, `.env.example`, `compose.yaml` |
